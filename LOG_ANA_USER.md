...
重试推理表位于页面最底部，每个 req_id 一行，按 user content 计算出的 sha256 前 32 位排序，并增加 `hash_id` 列。t1、t2 等列展示每次 attempt 的状态，推理失败状态使用黄色标注，最后一列展示该题评测结果是做对还是做错，表格使用紧凑行高。多次重试中只要有一次 attempt 成功，最终推理就按成功展示，并打开那次成功 attempt 的 JSON。点击状态符号或最终推理结果可以查看该 attempt 的 request/response JSON；搜索框支持按 req_id、hash_id、prompt、失败原因过滤。“只看过程失败”按钮会筛出过程中出现失败的题，包括最终失败和重试后成功的题；“只看做错”按钮会筛出评测结果为做错的题；“只看推理成功”按钮会筛出最终推理成功的题；“只看推理失败”按钮会筛出最终推理失败的题。
...
对比报告顶部展示两个 zip 文件名，并把核心指标箱线图按“同一指标一行”排列：每行左侧是文件 1 的箱线图，右侧是文件 2 的箱线图，例如 complete tokens 会在同一行左右并排比较。下方主体区域按模块分行对齐：基础信息、核心指标卡片、重试推理最终状态圆环图、异常摘要、response 长度点阵图和重试推理表都会各自形成一行，每行左侧是文件 1，右侧是文件 2，同一模块左右两格保持等高。两列的重试表筛选互不影响，点击任一列的状态点仍会打开对应 attempt 的 JSON 弹窗。
