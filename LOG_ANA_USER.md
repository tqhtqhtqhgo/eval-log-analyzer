...
response 长度点阵图中每道题是一个点，x 轴是 response 长度，y 轴按稳定 hash 做轻微错位以减少重叠，图高为原始点阵图高度的 3 倍。链路失败且 `eval_result=False` 的点显示为橙色。把光标放在点上可以看到 id、req_id、hash、长度和评测结果，点击点可以查看最终 attempt 的 JSON。
