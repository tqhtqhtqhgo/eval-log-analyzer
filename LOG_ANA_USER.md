...
重试链路表中每个 req_id 一行，t1、t2 等列展示每次 attempt 的状态，最后一列展示该题评测结果是做对还是做错，表格使用紧凑行高。多次重试中只要有一次 attempt 成功，最终链路就按成功展示，并打开那次成功 attempt 的 JSON。点击状态符号或最终链路结果可以查看该 attempt 的 request/response JSON；搜索框支持按 req_id、prompt、失败原因过滤。“只看过程失败”按钮会筛出过程中出现失败的题，包括最终失败和重试后成功的题；“只看做错”按钮会筛出评测结果为做错的题；“只看链路成功”按钮会筛出最终链路成功的题；“只看链路失败”按钮会筛出最终链路失败的题。
