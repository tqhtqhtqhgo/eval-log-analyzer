...
- 核心指标卡片：平均 token、平均耗时、retry、最终失败、最终失败 content 为空、empty、overlength、timeout；核心指标下方单独渲染完整数据箱线图，并为 complete tokens、reasoning tokens、content tokens 额外渲染过滤 0 后的非零箱线图，图中标注 min、1/4 分位、3/4 分位和 max。
...
- response 长度分布图：与重试链路表使用相同稳定排序和序号。做对/做错只根据 `export_data_list.json` 的 `eval_result` 判断：绿色表示做对，红色表示推理成功但评测做错，橙色表示推理失败或异常且最终评测做错。
