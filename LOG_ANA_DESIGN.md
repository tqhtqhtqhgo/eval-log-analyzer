...
按 request 中的 `hash_id` 分组。每组 response 长度取组内所有 req_id 最终 response_length 的平均值。正确次数从 `export_data_list.json` 中相同 req_id 的 `eval_result` 计算。总次数优先使用 `repeat_group_size`，为空时使用该 hash_id 下 req_id 数量。
...
- 核心指标卡片：平均 token、平均耗时、retry、最终失败、empty、overlength、timeout。
- 异常摘要表：empty、overlength、timeout、duplicate、parse_error、export_exception。
- 重试链路表：每个 req_id 一行，t1..tN 仅显示状态符号，不在主表展开 reasoning/content。
- response 长度分布图：与重试链路表共用 row_id，按最大长度归一化。
- hash_id 重复评测聚合图：仅在 `enable_hash_repeat_chart=True` 时显示。
