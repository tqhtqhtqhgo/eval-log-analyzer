...
重试链路表按 `ReqTrace.row_id` 保持稳定 id，每个 req_id 一行，t1..tN 仅显示成功/失败符号。点击 attempt 或最终结果时，内联 JS 从页面数据中读取对应 request/response JSON 并在弹窗展示；超过 50KB 默认截断，可手动显示完整 JSON。搜索字段匹配 req_id、prompt 和失败原因。

response 长度分布图与重试链路表共用 row_id。长度取最终 attempt 的 `response_length`，按页面内最大长度归一化。最终成功使用绿色条，最终失败使用红色条；点击行打开最终 attempt 的 JSON 弹窗，hover 展示 req_id、prompt、长度和最终结果。

hash_id 重复评测聚合图仅在 `enable_hash_repeat_chart=True` 时渲染。每行展示组 id、平均 response 长度和正确次数/总次数；正确次数为 0 使用红色，否则使用绿色。点击行会在弹窗展示 hash_id、组内 req_id、最终结果和最终 request/response 摘要。
