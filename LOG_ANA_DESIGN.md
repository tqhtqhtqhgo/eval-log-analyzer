...
- `metrics.py`：计算 export 指标、trace 指标、过程/最终异常摘要和 hash_id 聚合。
...
export 侧统计总条数、通过数、通过率、`eval_result` 分布、平均 complete/reasoning/content tokens、平均 used_time、平均 total_used_time、retry 条数、retry 成功条数、exception 条数和 exception 类型分布。过程异常摘要从 export 行的 `exception` / `exception_list` 文本统计：`Content OutOfMaxLength` 统计 exception 包含 `OutOfMaxLength` 的条数；`timeout` 统计 exception 以 `Streaming parse timeout` 开头的条数；`HTTP Connection 异常` 统计 exception 包含 `HTTPConnection` 的条数；`content_empty` 统计所有 attempt 中 response 的 `respMsg.content` 为空的条数。最终异常摘要只遍历最终推理失败的 `final_attempt`，按同样的异常文本规则统计 `OutOfMaxLength`、`Streaming parse timeout`、`HTTPConnection`，并统计最终失败 attempt 的 `content_empty`。基础信息中的通过题数、通过率，以及页面中“做对/做错”的判断都以 `export_data_list.json` 每个对象的 `eval_result` 为准，支持字符串 `"True"` / `"False"` 以及同义大小写写法。
...
trace 侧统计 req_id 总数、最终成功数、最终推理失败数、最终失败且 content 为空数量、其他最终失败数量、retry req_id 数量、retry 最终成功数量和 parse error 数量。核心指标展示最终推理失败数量和推理成功题目数量（不含推理失败），不再单独展示最终失败 content 为空数量。

empty、overlength、timeout 文件当前仅保持读取兼容，不再进入核心指标或过程异常摘要。duplicate 文件仍做基础存在性摘要，当前版本暂不做 COT 循环深度分析。
...
- 核心指标卡片：平均 token、平均耗时、retry、最终推理失败、推理成功题目数量（不含推理失败）；核心指标下方单独渲染完整数据箱线图，并为 complete tokens、reasoning tokens、content tokens 额外在“tokens推理成功数据”中渲染过滤 0 后的箱线图，图中标注 min、1/4 分位、3/4 分位和 max。content tokens 和 content tokens 非零按 0 到 64k 固定标尺绘制，used_time 和 total_used_time 按 0 到 5400 固定标尺绘制。
...
页面顶部展示两个文件名，并优先展示基础信息和核心指标卡片。核心指标箱线图从双列主体中提升到顶部统一对比，按指标卡片网格排列：每个指标卡片中左侧渲染文件 1 的箱线图，右侧渲染文件 2 的箱线图，图内只显示“文件 1 / 文件 2”，文件名保留在对比文件区和模块标题中。complete tokens、reasoning tokens、content tokens、used_time、total_used_time 使用全量箱线图；complete/reasoning/content tokens 继续额外展示过滤 0 后的“tokens推理成功数据”箱线图。箱线图的标尺规则与单报告一致。

对比报告主体按模块分行渲染，每行使用左右两格，左格对应文件 1，右格对应文件 2。基础信息、核心指标卡片、重试推理最终状态圆环图、过程异常摘要、最终异常摘要、response 长度点阵图、不含推理失败题目的 response 长度箱线图和重试推理表分别形成独立对比行，同一模块左右两格使用同一个 grid 行拉伸到等高，避免两个文件内容高度不一致时后续模块错位。对比报告中的 response 长度点阵图只展示点阵；做对/做错 response 长度箱线图从点阵图模块中拆出，单独放入等高对比行。两列的 attempt 弹窗数据使用不同前缀写入同一个内联 JS payload，避免相同 req_id 冲突；重试表的搜索和筛选按钮也使用独立 DOM 前缀，左右两列互不影响。
