...
- response 长度点阵图：每个 req_id 对应一个点，x 轴按 120k 固定满刻度展示 response 长度，画布宽度为普通图的 3 倍并可横向滚动，y 轴按稳定 hash 做轻微错位以减少重叠；链路失败且 `eval_result=False` 的点使用橙色；hover 显示 id、req_id、hash、长度和评测结果。
