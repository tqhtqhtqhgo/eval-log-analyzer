...
对比报告主体按模块分行渲染，每行使用左右两格，左格对应文件 1，右格对应文件 2。基础信息、核心指标卡片、重试推理最终状态圆环图、异常摘要、response 长度点阵图、不含推理失败题目的 response 长度箱线图和重试推理表分别形成独立对比行，同一模块左右两格使用同一个 grid 行拉伸到等高，避免两个文件内容高度不一致时后续模块错位。对比报告中的 response 长度点阵图只展示点阵；做对/做错 response 长度箱线图从点阵图模块中拆出，单独放入等高对比行。两列的 attempt 弹窗数据使用不同前缀写入同一个内联 JS payload，避免相同 req_id 冲突；重试表的搜索和筛选按钮也使用独立 DOM 前缀，左右两列互不影响。
